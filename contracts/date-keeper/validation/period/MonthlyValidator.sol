...
contract MonthlyValidator is PeriodValidator {
    using DateUtils for uint;

    ValidatorType public constant VALIDATOR_TYPE = ValidatorType.Monthly;

    bool public weekDayNumFromEnd;
    uint256 public weekDayNumOfMonth; // 1st, 2nd, 3rd, 4nd, 5nd week day number of the month
    ...
    function validateExpirationDate(uint256 expirationDate) public view returns (bool) {
        ...
        uint256 validExpirationDate = getNextValidExpirationDateWithFullHours(
            now.add(minIntervalSinceDeployment)
        );
        validExpirationDate = DateUtils.setFullHour(validExpirationDate, validHour);
        if (validExpirationDate < now.add(minIntervalSinceDeployment)) {
            validExpirationDate = getNextValidExpirationDateWithFullHours(
                DateUtils.addMonths(validExpirationDate, 1)
            );
        }
        ...
        for (uint256 i = 0; i < periodInTheFutureEnabled; i++) {
            ...
            validExpirationDate = DateUtils.addMonths(validExpirationDate, 1);
            validExpirationDate = getNextValidExpirationDateWithFullHours(
                validExpirationDate
            );
        }
